...
export class ProveedoresController {
  ...
  async updateProveedor(
    @Param('id') id_proveedor: string,
    @Body() datos: ProveedoresDTO,
  ) {
    return await this.proveedoresService.updateProveedores(
      Number(id_proveedor),
      datos,
    );
  }
  ...
  async deleteProveedores(@Param('id') id_proveedor: string) {
    return await this.proveedoresService.deleteProveedores(
      Number(id_proveedor),
    );
  }
}
